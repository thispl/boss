...
def execute(filters=None):
    ...
    for ss in salary_slips:
        row = [ss.name, ss.employee, ss.employee_name, doj_map.get(ss.employee), ss.client_name, ss.site, ss.designation,
            ss.company, ss.start_date, ss.end_date, ss.total_working_days, ss.payment_days,ss.total_working_hours]

        if not ss.client_name == None:columns[3] = columns[3].replace('-1','120')
        if not ss.site  == None: columns[4] = columns[4].replace('-1','120')
...
def get_columns(salary_slips):
    ...
    columns = [
        ...
        _("Date of Joining") + "::80", _("Client") + ":Link/Client:120", _("Site") + ":Link/Site:120",
        _("Designation") + ":Link/Designation:120", _("Company") + ":Link/Company:120", _("Start Date") + "::80",
        _("End Date") + "::80", _("Total Working Days") + ":Float:130", _("Payment Days") + ":Float:120", _("Total Working Hours") + ":Float:120"
    ]
    """
    columns = [
        _("Salary Slip ID") + ":Link/Salary Slip:150",_("Employee") + ":Link/Employee:120", _("Employee Name") + "::140",
        _("Date of Joining") + "::80", _("Client") + ":Link/Client:-1", _("Site") + ":Link/Site:-1",
        _("Designation") + ":Link/Designation:-1", _("Company") + ":Link/Company:120", _("Start Date") + "::80",
        _("End Date") + "::80", _("Total Working Days") + ":Float:-1", _("Payment Days") + ":Float:120" , _("Total Working Hours") + ":Float:120"
    ]
