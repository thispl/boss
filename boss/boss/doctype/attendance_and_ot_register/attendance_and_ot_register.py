...
class AttendanceandOTRegister(Document):
    ...
    def create_advance_deduction(self):
        additional_salary = frappe.db.get_value("Additional Salary",{"employee":self.employee,"salary_component": "Salary Advance","payroll_date": self.start_date},["name"])
        if not additional_salary:
            if self.advance_deduction > 0:
                ...
                additional_salary.update({
                    ...
                    "amount": int(self.advance_deduction),
                    "overwrite_salary_structure_amount": "1"
                })
                additional_salary.save(ignore_permissions=True)
                additional_salary.submit()
...
@frappe.whitelist()
def create_salary_advance(advance,payroll_date,employee):
    additional_salary = frappe.db.get_value("Additional Salary",{"employee":employee,"salary_component": "Salary Advance","payroll_date": payroll_date},["name"])
    if not additional_salary:
        additional_salary = frappe.new_doc('Additional Salary')
        additional_salary.update({
            "employee": employee,
