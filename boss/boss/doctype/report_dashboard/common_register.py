...
@frappe.whitelist()
def title7(args):
    data=['Name and Location of Work : Boss Management Services Pvt Ltd','','','','','','']
    # data.append(args.client)
    return data

@frappe.whitelist()
def title8(args):
    data=['Sr no','ID No','Name of Workman','No of days Worked','Basic wages','HRA','Overtime HRS','OT Amount','ATT Bonus','WA','Total','P.F','E.S.I','Canteen','Advance','PT','Total Deduction','Net Amount Paid','Sign or Thumb Impression of Workman',]
    return data
...
def get_data(args):
    data=[]
    values = frappe.db.sql("select name,`tabSalary Slip`.employee_name as employee_name,`tabSalary Slip`.client_employee_no as employee,`tabSalary Slip`.payment_days as payment_days,`tabSalary Slip`.rounded_total as rounded,`tabSalary Slip`.total_deduction as deduction,`tabSalary Slip`.net_pay as net_pay ,`tabSalary Slip`.total_working_hours as total_wh from `tabSalary Slip` where `tabSalary Slip`.start_date between '%s'and '%s'"%(args.from_date,args.to_date),as_dict=True)
    if args.company:
        values = frappe.db.sql("select name,`tabSalary Slip`.employee_name as employee_name,`tabSalary Slip`.client_employee_no as employee,`tabSalary Slip`.payment_days as payment_days,`tabSalary Slip`.rounded_total as rounded,`tabSalary Slip`.total_deduction as deduction,`tabSalary Slip`.net_pay as net_pay,`tabSalary Slip`.total_working_hours as total_wh from `tabSalary Slip` where `tabSalary Slip`.company = '%s' and `tabSalary Slip`.client_name = '%s' and `tabSalary Slip`.start_date between '%s'and '%s'"%(args.company,args.client,args.from_date,args.to_date),as_dict=True)
    i = 1
    for value in values:
        row = []
        row.extend([i,value.employee,value.employee_name,value.payment_days,])
        ...
        pf_no = frappe.db.get_value("Employee",{'client_employee_no':value.employee},['bank_ac_no'])

        row.append(basic)
        row.append(house_all)
        row.append(value.total_wh)
        row.append(overtime)
        row.append(att_bonus)
        row.append(wa)
...
# @frappe.whitelist()
# def title8(args):
#     data=['Sr no','Employee No','Employee Name','Role','DOJ','No of days Worked','Basic wages','DA','Leave with Wages','Advance Bonus','HRA','Other allowance','OT Amount','Overtime HRS','Total','P.F','E.S.I','Food','PT','Salary Advance','Salary Shoe & Uniform','Total Deduction','Travel allowance','Net Amount Paid','Sign or Thumb Impression of Workman',]
#     return data
...
#         advance = frappe.db.get_value("Salary Detail",{'abbr':'AB_1','parent':value.name},'amount')
#         pt = frappe.db.get_value("Salary Detail",{'abbr':'PT','parent':value.name},'amount')
#         pf_no = frappe.db.get_value("Employee",{'employee':value.employee},['bank_ac_no','designation'])
#         row.append(pf_no.designation)
